...
integer(i4) :: m, mref, s, tcref, isoref
...
DO p=1, npoolcan
   cp = pool_indx_can(p) !live pool indexed by canopy (1,4,5)
   cpc13 = pool_indx_can(p)+5 !(6,9,10) poollt
...
!   poollt%loss_grzc13(p) = fract%rcpoolfac * poollt%loss_grz(p)
...
!DO p=1, npoolcanc13 !12,15,16
!   cpc13 = pool_indx_canc13(p)-6 !(12,15,16)-6=(6,9,10) live pool
!   tcref = pool_indx_canc13(p)-11 !(12,15,16)-11=(1,4,5)
...
!!   poollt%loss_grzc13(p) = poollt%rcpoolpft(cpc13) * poollt%loss_grz(tcref)
!   poollt%loss_grzc13(p) = fract%rcpoolfac * poollt%loss_grz(tcref)
!   poollt%poolpft_dloss(cpc13,1) = &
!       poollt%poolpft_dloss(cpc13,1) + poollt%loss_grzc13(p)*dtsib
!ENDDO
