...
if (sibg%gprogt%firec .gt. dzero) then
   ...
   ntpft = sibg%g_nlu !with C-13 pools, ntpft is half the total
   ! number of land units/PFTs per cell -> =1 for Hyy when run as 1.0ENF
   ! ntpft=ntpft_temp/2
   !...Fire Carbon error during run: try leaving as is
   !...13C pools should be small enough that they won't be considered 
