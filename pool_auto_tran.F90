...
do n=1,npoolpft/2 !(1,5)
   ...
   do s=1,pool_indx_lay(n) !1,pool_indx_lay(1,5) either 1 or 10
       ...
       !poolpft_avail_lay(isoref,s) = &
       !      fract%rcpoolfac * poolpft_avail_lay(n,s)
       poolpft_avail_lay(isoref,s) = &
             poollt%rcpoolpft_lay(isoref,s) * poolpft_avail_lay(n,s)

       poollt%pftavail_lay(isoref,s) = poolpft_avail_lay(isoref,s)


!! print statements to check poolpft_loss and poolpft_lay
!if ((poollt%rcpoolpft_lay(isoref,s) .gt. 1.)) then
if ( (poollt%poolpft_dloss(isoref,s) .gt. 10.) .or. &
            (poollt%poolpft_dloss(isoref,s) .lt. -10.)) then
    print*,' '
    print*,'code: pool_auto_tran'
    print*,'isoref,s: ',isoref,s
    print*,'poolpft_dloss(isoref,s/s+1):',poollt%poolpft_dloss(isoref,s),poollt%poolpft_dloss(isoref,s+1)
    print*,'poolpft_lay(isoref,s/s+1) :',poollt%poolpft_lay(isoref,s),poollt%poolpft_lay(isoref,s+1)
    print*,' '
endif
...
do n=1, npoolpft/2 !1,5
   ...
   do s=1,pool_indx_lay(n) !1,pool_indx_lay(1,5) either 1 or 10
      ...
      !if (poolpft_avail_lay(n,s) .gt. dzero) then
        ...
        !tloss_lay(isoref,s) = fract%rcpoolfac * tloss_lay(n,s) 
        tloss_lay(isoref,s) = poollt%rcpoolpft_lay(isoref,s) * tloss_lay(n,s)
        tfrac(isoref) = tfrac(n)
      endif
   enddo
   poollt%tf_turnover(n) = tfrac(n)*steps_per_day
   poollt%tf_turnover(isoref) = tfrac(isoref)*steps_per_day
enddo
...
!   nref=n+npoolpft/2+1 !(12,16)
!   do s=1,pool_indx_lay(nref) !1,pool_indx_lay(12,16) either 1 or 10
!!      if (tloss_lay(tcref,s) .gt. dzero) then
!!      if (poolpft_avail_lay(tcref,s) .gt. dzero) then !(npoolpft,nsoil)
!        tloss_lay(n,s) = (1 - poolcont%lresp_eff(n))  &
!           * poollt%krater_lay(n,s) * poolpft_avail_lay(n,s) * dtsib
!      !tloss_lay(npoolpft,nsoil)
!      !  tloss_lay(n,s) = fract%rcpoolfac * tloss_lay(tcref,s) 
!      !if (poolpft_avail_lay(tcref,s) .gt. dzero) then !(npoolpft,nsoil)
!      !  tfrac(n) = tfrac(n) &
!      !         + poollt%poolpft_flay(n,s) * tloss_lay(n,s) &
!      !         / poolpft_avail_lay(n,s)
